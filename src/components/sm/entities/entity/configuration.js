...
let resolvePropertyDerivation     = function (propertyConfig, identitySmEntity) {
	Object.entries(propertyConfig.derivedFrom)
	      .forEach(([name, derivedFrom]) => {
		      if (!(derivedFrom === true || typeof derivedFrom === "string")) {
			      throw new Error("Cannot derive properties any other way");
		      }

		      const model        = identitySmEntity;
		      const propertyName = typeof derivedFrom === "string" ? derivedFrom : name;
		      ...
		      switch (typeof derivedFrom) {
			      case "string":
				      console.log(property);
				      propertyConfig.datatypes = propertyConfig.identity instanceof Identity ? propertyConfig.identity : identitySmEntity[SM_ID];
				      return propertyConfig.derivedFrom[name] = property[SM_ID];
			      default:
